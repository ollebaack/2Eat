...
export function RecipeCard({ recipe, onDelete }: RecipeCardProps) {
  ...
  return (
    ...
    >
      <Link to={`/recipes/${recipe.id}`} className="block relative no-underline">
        <PhotoSlot
        ...
        <Button
          variant="ghost"
          size="icon"
          className="absolute top-3 right-3 h-8 w-8 rounded-full bg-white/90 backdrop-blur-sm hover:bg-white/95"
          style={{ border: 'none' }}
      ...
      <div className="p-[18px] flex flex-col gap-3 flex-1">
        <div className="flex items-start justify-between gap-3">
          <Link to={`/recipes/${recipe.id}`} className="no-underline flex-1">
            <h3
